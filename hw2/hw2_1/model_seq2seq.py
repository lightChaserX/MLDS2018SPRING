...
class VideoCaptionGenerator():
	def __init__(self, image_dim, num_of_words, hidden_dim, batch_size, num_of_lstm_steps, num_of_video_lstm_steps, 
		...
		#	self.Wemb = tf.Variable(tf.random_uniform([num_of_words, hidden_dim], -0.1, 0.1), name = 'Wemb')
		...
		#self.encode_image_w = tf.Variable(tf.random_uniform([image_dim, hidden_dim], -0.1, 0.1), name='encode_image_w' )
		#self.encode_image_b = tf.Variable(tf.zeros([hidden_dim]), name='encode_image_b')

		#self.embedding_word_w = tf.Variable(tf.random_uniform([hidden_dim, num_of_words], -0.1, 0.1), name='embedding_word_w')
		#if bias_init_vector is not None:
		#	self.embedding_word_b = tf.Variable(bias_init_vector.astype(np.float32), name='embedding_word_b')
		#else:
		#	self.embedding_word_b = tf.Variable(tf.zeros([num_of_words]), name='embedding_word_b')

	def build_model(self):
		video_feature = tf.placeholder(tf.float32, [self.batch_size, self.num_of_video_lstm_steps, self.image_dim])
		#video_mask = tf.placeholder(tf.float32, [self.batch_size, self.num_of_video_lstm_steps])

		caption = tf.placeholder(tf.int32, [self.batch_size, self.num_of_caption_lstm_steps+2])
		
		#print(caption.shape)
		#print(self.num_of_caption_lstm_steps)
		...
		decoder = tf.contrib.seq2seq.BasicDecoder(decoder_cell, helper, 
			decoder_cell.zero_state( self.batch_size, tf.float32).clone(cell_state=encoder_state), output_layer = projection_layer)
